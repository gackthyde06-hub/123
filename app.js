...
function openTime(iso) {
  if (!iso) return '開倉時間未知';
  try {
    return '開倉 ' + new Date(iso).toLocaleString('zh-TW', {
...
function traderCard(t) {
  ...
  const expanded = expandedTraders.has(t.id);

  const allPositions = [...(t.positions || [])].sort((a, b) => {
    const ta = a.openTime ? new Date(a.openTime).getTime() : 0;
    const tb = b.openTime ? new Date(b.openTime).getTime() : 0;
    return tb - ta;
  });
  const visiblePositions = expanded
    ? allPositions
    : allPositions.slice(0, DEFAULT_VISIBLE_POSITIONS);

  const positions = allPositions.length
    ? visiblePositions.map(p => {
        ...
        return `<div class="pos">
          <div>
            ...
            <div class="opened">${esc(openTime(p.openTime))}</div>
          </div>
          <div class="price"><span>進場均價</span><b>${price(p.entryPrice)}</b></div>
        </div>`;
      }).join('')
    : '<div class="empty">目前沒有公開倉位</div>';

  const expandButton = allPositions.length > DEFAULT_VISIBLE_POSITIONS
    ? `<button class="expandBtn" data-expand="${esc(t.id)}">${
        expanded ? '收合倉位' : `查看全部 ${allPositions.length} 個倉位`
      }</button>`
    : '';
  ...
  return `<section class="traderCard">
    <div class="traderHead">
      <div>
        ...
        <div class="traderMeta">${esc(t.role)} · <span class="${ok ? 'okText' : 'badText'}">${ok ? (t.baselineReady ? '監控中' : '建立基準') : '讀取異常'}</span> · ${allPositions.length} 倉</div>
      </div>
...
function renderTraders(list) {
  ...
  document.querySelectorAll('.expandBtn').forEach(el => {
    el.addEventListener('click', e => {
      const id = e.currentTarget.dataset.expand;
      if (expandedTraders.has(id)) expandedTraders.delete(id);
      else expandedTraders.add(id);
      renderTraders(lastStatus?.traders || list);
    });
  });
}
