...
function must(p){if(!fs.existsSync(p))throw new Error(`[v2626-status] missing ${path.basename(p)}`);return p}
function check(file){const r=spawnSync(process.execPath,['--check',file],{encoding:'utf8'});if(r.status!==0)throw new Error(`[v2626-status] syntax invalid ${path.basename(file)}: ${String(r.stderr||r.stdout||'').trim()}`)}
export function patchGrowthStatusV2626(){
  ...
  s=s.replace('</head>','<link rel="stylesheet" href="/growth-status-v2626.css?v=2626">\n</head>');
...
if(import.meta.url===`file://${process.argv[1]}`)console.log(patchGrowthStatusV2626());
