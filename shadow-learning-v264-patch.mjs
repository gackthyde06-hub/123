...
const MARKER='ABC_SHADOW_LEARNING_V265_20260902';
const OLD_MARKER='ABC_SHADOW_LEARNING_V264_20260902';
function replaceOnce(src,oldText,newText,label){if(!src.includes(oldText))throw new Error(`[abc-shadow-v265] anchor missing: ${label}`);return src.replace(oldText,newText)}
function check(file){const r=spawnSync(process.execPath,['--check',file],{encoding:'utf8'});if(r.status!==0)throw new Error(`[abc-shadow-v265] syntax invalid: ${String(r.stderr||r.stdout||'').trim()}`)}
export function patchShadowLearningV264({serverPath=path.join(__dirname,'server.js')}={}){
  ...
  const upgrading=src.includes(OLD_MARKER);
  if(upgrading){
    const start=src.indexOf('/* '+OLD_MARKER),end=src.indexOf(insertAnchor,start);
    if(start<0||end<0)throw new Error('[abc-shadow-v265] V2.6.4 block boundary missing');
    src=src.slice(0,start)+code+src.slice(end);
    src=src.replace("version:'V2.6.4',generatedAt", "version:'V2.6.5',generatedAt");
    src=src.replace('Structure V2＋全自動 ABC Shadow＋資料新鮮度＋TP2 RR。即使不下單，符合可驗證點位的 A/B/C 仍會進 Shadow；去相關後才影響學習與通知分數。','Structure V2＋全自動 ABC Shadow＋資料新鮮度＋TP2 RR。未確認候選若尚無 TP1，研究影子會用成本到 SP 的等距 1R 作追蹤目標；不改實際建議 TP，也不放寬通知硬門檻。');
  }else{
  ...
  const csvNew="const cols=['shadowAt','shadowSource','manualGradeAtEntry','manualGradeCurrent','manualExecutionScoreAtEntry','abcTargetDerived','abcEntryPriceSource','abcReferenceEntryPrice','symbol','direction','strategyLabel','marketRegime','stateLabel','tierAtEntry','finalTier','notified','learningEligible','shadowProgress','blockReasons','stateKeyCore','stateKeyBroad','rawScore','learningAdjustmentAtEntry','adjustedScore','entryPrice','stop','target','status','result','resultAt','mfePct','maePct','realizedR','grossReturnPct','netReturnPct'];";
  if(src.includes(csvOld))src=src.replace(csvOld,csvNew);
  else if(src.includes("'manualExecutionScoreAtEntry','symbol'"))src=src.replace("'manualExecutionScoreAtEntry','symbol'","'manualExecutionScoreAtEntry','abcTargetDerived','abcEntryPriceSource','abcReferenceEntryPrice','symbol'");
  const tmp=`${serverPath}.v265-${process.pid}-${Date.now()}.tmp.js`;fs.writeFileSync(tmp,src,'utf8');try{check(tmp);fs.renameSync(tmp,serverPath)}catch(e){try{fs.unlinkSync(tmp)}catch{};throw e}
  return {changed:true};
}
