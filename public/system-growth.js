...
(()=>{
  ...
  function restoreGrowthAnchor(root,a){if(!a||window.uiMotionHeldV2617?.())return;requestAnimationFrame(()=>requestAnimationFrame(()=>{if(window.uiMotionHeldV2617?.())return;let el=a.external&&a.node?.isConnected?a.node:null;if(!el&&a.key)for(const x of root.querySelectorAll('[data-sg-detail-key],[data-sg-candidate],#sgAbcArena,#sgCandidateSection,#sgLessonSection,.sg-explore,.sg-attributes-layout,.sg-stage,.sg-core-card')){if(detailKey(x)===a.key||x.id===a.key){el=x;break}}if(!el)return;const delta=el.getBoundingClientRect().top-a.top;if(Math.abs(delta)>8&&Math.abs(delta)<window.innerHeight*1.5)window.scrollBy({top:delta,left:0,behavior:'auto'})}))}
  ...
  function getMetrics(){
    const perf=state.perf||{},sh=perf.shadowSummary||{},sum=perf.summary||{},research=perf.research||{},patterns=perf.stateLearning?.patterns||[],rows=state.signals?.rows||[];
    ...
    return {perf,sh,sum,research,patterns,rows,xp,level,stage,attrs,tierCounts,blockerTop,personality,best,worst,selectivity};
  }
  ...
  function lessonHtml(m){const l=lessonFor(m),parts=String(l.title||'').split('｜');return `<details class="sg-lesson" data-sg-lesson data-sg-detail-key="lesson"><summary><div><span>${esc(l.tag)}</span><b class="sg-lesson-title">${parts.map(x=>`<span>${esc(x)}</span>`).join('')}</b><small>${esc(l.lead)}</small></div><i>展開</i></summary><div class="sg-lesson-body"><div class="sg-lesson-current"><span>目前重點</span><b>${esc(l.focus)}</b></div><ol>${l.points.map(x=>`<li>${esc(x)}</li>`).join('')}</ol></div></details>`}

  function researchAuditHtml(r){
    if(!r||!r.revision)return `<div class="sg-research-audit sg-research-audit-wait"><span>研究層</span><b>等待後端 Research Layer</b><small>核心交易規則不受影響</small></div>`;
    const warnings=Array.isArray(r.warnings)?r.warnings:[],high=warnings.filter(x=>x?.severity==='HIGH').length,medium=warnings.filter(x=>x?.severity==='MEDIUM').length;
    const netPf=has(r?.overall?.netProfitFactor)?Number(r.overall.netProfitFactor).toFixed(2):'—';
    const netExp=has(r?.overall?.netExpectancyR)?`${Number(r.overall.netExpectancyR)>=0?'+':''}${Number(r.overall.netExpectancyR).toFixed(3)}R`:'—';
    const scoreCorr=has(r?.score?.outcomeCorrelation)?Number(r.score.outcomeCorrelation).toFixed(3):'—';
    const coreReady=Number(r?.hierarchy?.coreReady||0),maxCore=Number(r?.hierarchy?.maxCoreSample||0),date=String(r.releaseDate||r?.audit?.release?.releaseDate||'—');
    return `<div class="sg-research-audit"><div class="sg-research-audit-head"><span>RESEARCH AUDIT</span><b>${esc(r.revision)}</b><small>${esc(date)} · CORE ${r.coreRulesChanged?'CHANGED':'FROZEN'}</small></div><div class="sg-research-audit-grid"><div><span>分數定義</span><b>結構完成度</b><small>100 ≠ 100% 勝率</small></div><div><span>成本後</span><b>Net PF ${esc(netPf)}</b><small>Expectancy ${esc(netExp)}</small></div><div><span>Score→結果</span><b>${esc(scoreCorr)}</b><small>越接近 0 越無辨識力</small></div><div><span>核心狀態桶</span><b>${coreReady} READY</b><small>最大樣本 ${maxCore}</small></div></div><div class="sg-research-audit-flags">${high?`<i class="bad">高優先 ${high}</i>`:''}${medium?`<i>觀察 ${medium}</i>`:''}${!warnings.length?'<i class="good">目前無研究警示</i>':''}<i class="freeze">通知門檻未放寬</i></div></div>`;
  }
  ...
  function render(){
    ...
    panel.innerHTML=`
      ...
      <section id="sgAbcArena" class="sg-abc-arena" aria-label="影子戰術"><div class="sg-abc-loading"><b>影子戰術</b><span>影子正在整理戰況…</span><i></i><i></i><i></i></div></section>
      ${explorationHtml()}
      <section id="sgLessonSection">${lessonHtml(m)}</section>
      <section class="sg-live" id="sgCandidateSection"><div class="sg-section-head"><div><b>篩選中</b><span>LIVE RESEARCH</span></div><span>${rows.length} 個觀察狀態</span></div>${candidateHtml(rows)}</section>
      <div class="sg-accordions">
        <details class="sg-accordion" data-sg-detail-key="skill-tree" open><summary><i class="sg-acc-icon">${sigilSvg('state',2)}</i><div><b>技能樹</b><span>SKILL BOARD</span></div><em>6 PATHS</em></summary><div class="sg-detail-body"><div class="sg-skills sg-skill-tree">
          ${skillCard('return','回踩獵手','辨識順勢回踩是否值得等，專看不同 Regime 的回踩勝率與延續性',skillReturn,'回踩模式樣本')}${skillCard('break','破局之眼','研究突破後的回測是否站穩，避免追到假突破或事件沖高回落',skillBreak,'突破模式樣本')}${skillCard('shadow','影子研究','把沒通知的樣本也納入研究，避免只記得贏單，降低主觀偏誤',m.xp.effective,'去相關有效樣本')}${skillCard('state','狀態學習','先看策略 × Regime × 方向的核心桶，再用 OI / Taker / Depth 做細分；樣本不足不自動調權',stateResearch,'模式有效樣本')}${skillCard('depth','流動性雷達','觀察掛單厚度、價差與流動性，避開容易滑價或被掃的區域',depthResearch,'Depth 模式樣本')}${skillCard('risk','危機預警','累積被擋樣本與反證訊號，提醒哪些情況看起來漂亮其實不該出手',m.xp.blocked,'被擋樣本')}
        </div></div></details>
        <details class="sg-accordion" id="sgJournal" data-sg-detail-key="journal"><summary><i class="sg-acc-icon">${sigilSvg('journal',2)}</i><div><b>模型日誌</b><span>MODEL LOG</span></div><em>${m.research?.revision||`${patterns.length} ACTIVE`}</em></summary><div class="sg-detail-body">${researchAuditHtml(m.research)}<div class="sg-journal-grid"><div class="good"><span>目前最強模式</span><b>${esc(patternText(m.best))}</b></div><div class="bad"><span>目前最弱模式</span><b>${esc(patternText(m.worst))}</b></div></div><div class="sg-journal-note"><span>目前最常阻擋</span><div>${m.blockerTop.length?m.blockerTop.map(([k,v])=>`<i>${esc(k)} <b>${v}</b></i>`).join(''):'<i>暫無集中風險</i>'}</div></div><div class="sg-journal-note"><span>今日狀態</span><b>${m.tierCounts.HIGH+m.tierCounts.NORMAL>0?`READY · ${m.tierCounts.HIGH+m.tierCounts.NORMAL} 通知級`:'WAIT'}</b></div></div></details>
        ...
        <details class="sg-accordion" data-sg-detail-key="codex"><summary><i class="sg-acc-icon">${sigilSvg('codex',2)}</i><div><b>研究手冊</b><span>CODEX</span></div><em>6 FILES</em></summary><div class="sg-detail-body"><div class="sg-manual"><details data-sg-detail-key="codex-dedup"><summary>45 分去相關，到底在防什麼？</summary><p>同一段行情會長出很多長得很像的訊號。全部算進去會灌水，所以系統把太近的樣本視為同群，讓去相關有效樣本更接近真正研究量。</p></details><details data-sg-detail-key="codex-pf"><summary>PF 為什麼比勝率更重要？</summary><p>PF = 總獲利 ÷ 總虧損。勝率不是全部；只要賺賠比夠好，PF 一樣能贏。</p></details><details data-sg-detail-key="codex-mfe"><summary>MFE / MAE 是什麼？</summary><p>MFE 看最多順著你跑多遠；MAE 看最多逆著你跑多深。拿來修 TP / SL 很實用。</p></details><details data-sg-detail-key="codex-state"><summary>狀態學習是在學什麼？</summary><p>不是學哪顆幣一定漲，而是比較策略、方向、Regime、OI、Taker、Depth 的組合，哪些長期更強。</p></details><details data-sg-detail-key="codex-score"><summary>100 分代表勝率 100% 嗎？</summary><p>不是。Raw / Adjusted Score 是條件與結構完成度；真正的辨識力要看不同分數桶在去相關新樣本中的命中、PF 與成本後期望。</p></details><details data-sg-detail-key="codex-net"><summary>為什麼現在多看 Net PF？</summary><p>方向先到 TP 不代表經濟上值得做。Net PF / Net Expectancy 把手續費與固定成本拖累納入，避免學到「方向對但賺不到錢」的訊號。</p></details></div></div></details>
  ...
  function intelListHtml(rows){const list=candidateRows(rows).slice(0,3);if(!list.length)return'<div class="sg-empty">目前沒有候選情報檔案。</div>';return list.map(x=>`<article class="sg-intel" data-sg-intel="${esc(x.symbol)}:${esc(x.direction)}"><div class="sg-intel-head"><div>${sgTvAnchor(x.symbol)}<span>${x.direction==='SHORT'?'做空':'做多'} · ${esc(x.strategyAtConfirm?.label||x.strategyProfile?.label||'多策略')}</span></div><button type="button" class="sg-intel-btn" data-sg-intel-btn data-symbol="${esc(x.symbol)}" data-direction="${esc(x.direction)}">查最新情報</button></div><div class="sg-intel-body" data-sg-intel-body>尚未查詢</div></article>`).join('')}
