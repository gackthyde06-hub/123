...
function renderTestCrossDetail(x){
  const bt=x.setup?.backtest||{},lc=x.lastCheck||{},ev=x.monitorEvidence||{},dh=x.dataHealth||{},src=dh.sources||{},long=x.direction==='LONG',win=testEffectiveWinRate(x),confirmed=hasNum(x.confirmedWinRate)?Number(x.confirmedWinRate):null;
  const srcLabel=(key,label)=>`<span class="sourceChip ${src[key]?'ok':'miss'}">${esc(label)} ${src[key]?'✓':'無資料'}</span>`;
  const sourceHealth=`<div class="testSourceHealth"><div class="sourceHealthHead"><b>資料完整度 ${hasNum(dh.coveragePct)?Number(dh.coveragePct).toFixed(0)+'%':'—'}</b><span>缺資料不會用 0 / 中性假裝有效</span></div><div class="sourceChips">${srcLabel('k5','5分K')}${srcLabel('k15','15分K')}${srcLabel('k30','30分K')}${srcLabel('h1','1小時K')}${srcLabel('oi','OI')}${srcLabel('taker','主動買賣')}${srcLabel('topPos','大戶持倉')}${srcLabel('globalLs','全市場多空')}${srcLabel('depth','20檔委託簿')}${srcLabel('funding','Funding')}${srcLabel('basis','Basis')}${srcLabel('adl','ADL Risk')}${srcLabel('mark','Mark Price')}${srcLabel('market','BTC/ETH大盤')}${srcLabel('backtest','回測')}</div><small>ADL Risk 走 Binance 公開端點（約30分鐘更新）；任何來源抓不到都直接標示無資料，不用中性值冒充。</small></div>`;
  ...
  <div class="testCrossSection"><h4>多週期結構 / 動能</h4><div class="testCrossGrid"><div><span>5分 RSI / ADX</span><b>${src.k5&&hasNum(lc.rsi5)?Number(lc.rsi5).toFixed(1):'無資料'} / ${src.k5&&hasNum(lc.adx5)?Number(lc.adx5).toFixed(1):'—'}</b></div><div><span>15分 RSI / ADX</span><b>${src.k15&&hasNum(lc.rsi15)?Number(lc.rsi15).toFixed(1):'無資料'} / ${src.k15&&hasNum(lc.adx15)?Number(lc.adx15).toFixed(1):hasNum(x.setup?.adx15)?Number(x.setup.adx15).toFixed(1):'—'}</b></div><div><span>30分方向</span><b>${src.k30?testTrendWord(lc.t30Trend):'無資料'}</b></div><div><span>1小時方向</span><b>${src.h1?testTrendWord(lc.h1Trend):'無資料'}</b></div><div><span>5分 / 15分量比</span><b>${src.k5&&hasNum(lc.volumeRatio)?Number(lc.volumeRatio).toFixed(2):'無資料'} / ${src.k15&&hasNum(lc.volumeRatio15)?Number(lc.volumeRatio15).toFixed(2):'—'}</b></div><div><span>MACD改善 / 15分柱</span><b>${testBoolWord(lc.macdImprove)} / ${src.k15&&hasNum(lc.macd15)?Number(lc.macd15).toPrecision(3):'—'}</b></div></div></div>
  <div class="testCrossSection"><h4>資金 / 衍生品</h4><div class="testCrossGrid"><div><span>OI變化</span><b>${src.oi?testSignedPct(lc.oiChangePct,1):'無資料'}</b></div><div><span>主動買賣比</span><b>${src.taker&&hasNum(lc.takerRatio)?Number(lc.takerRatio).toFixed(2):'無資料'}</b></div><div><span>大戶持倉比</span><b>${src.topPos?top:'無資料'}</b></div><div><span>全市場多空比</span><b>${src.globalLs?globalLs:'無資料'}</b></div><div><span>Funding / Basis</span><b>${src.funding?fund:'無資料'} / ${src.basis&&hasNum(lc.basisPct)?testSignedPct(lc.basisPct,3):'無資料'}</b></div><div><span>ADL Risk</span><b>${src.adl&&lc.adlRisk&&lc.adlRisk!=='unknown'?esc(String(lc.adlRisk).toUpperCase()):'無資料'}</b></div></div></div>
  <div class="testCrossSection"><h4>委託簿 / 進場品質</h4><div class="testCrossGrid"><div><span>20檔深度失衡</span><b>${src.depth?depth:'無資料'}</b></div><div><span>價差</span><b>${src.depth&&hasNum(lc.spreadBps)?Number(lc.spreadBps).toFixed(2)+' bps':'無資料'}</b></div><div><span>追價距離</span><b>${hasNum(lc.chaseAtr)?Number(lc.chaseAtr).toFixed(2)+' ATR':'—'}</b></div><div><span>BTC/ETH對齊</span><b>${src.market?(Number(lc.marketAlign)>0?'同向':Number(lc.marketAlign)<0?'逆向':'中性'):'無資料'}</b></div><div class="wide"><span>較佳進場區</span><b>${esc(zone)}</b></div><div><span>保護 / 失效</span><b>${protect}</b></div></div></div>
...
function renderTestSignals(d){
  ...
  if($('testHealth'))$('testHealth').textContent=`系統 ${healthText} · 最近掃描 ${scanSec==null?'—':scanSec+'秒前'} · 監控 ${Number(h.tracked||0)} · 免費資料完整度會影響通知級別`;
