...
const CALC_PREF='position-alert-order-calc-v595';
...
function saveCalc(){const o={margin:$('calcMargin')?.value||'',lev:$('calcLev')?.value||'',symbol:$('calcSymbol')?.value||'',side:$('calcSide')?.value||'LONG',entry:$('calcEntry')?.value||'',tp:$('calcTp')?.value||'',sl:$('calcSl')?.value||'',open:$('tradeCalc')?.open||false};try{localStorage.setItem(CALC_PREF,JSON.stringify(o))}catch{}}
function loadCalc(){const d=loadObject(CALC_PREF,{});if($('calcMargin'))$('calcMargin').value=d.margin||'';if($('calcLev'))$('calcLev').value=d.lev||'';if($('calcSymbol'))$('calcSymbol').value=d.symbol||'';if($('calcSide'))$('calcSide').value=d.side==='SHORT'?'SHORT':'LONG';if($('calcEntry'))$('calcEntry').value=d.entry||'';if($('calcTp'))$('calcTp').value=d.tp||'';if($('calcSl'))$('calcSl').value=d.sl||'';if($('tradeCalc'))$('tradeCalc').open=!!d.open;updateCalc()}
function updateCalc(){
...
function fillCalcFromPosition(el){if(!el)return;$('calcSymbol').value=String(el.dataset.calcSymbol||'').toUpperCase();$('calcSide').value=el.dataset.calcSide==='SHORT'?'SHORT':'LONG';$('calcEntry').value=el.dataset.calcEntry||'';$('tradeCalc').open=true;updateCalc();$('calcMsg').innerHTML=`<span class="calcSelected">已帶入 ${esc(el.dataset.calcSymbol||'')} ${el.dataset.calcSide==='SHORT'?'做空':'做多'} 進場位</span> · 再填 TP / SL 即可試算`;saveCalc();$('tradeCalc').scrollIntoView({behavior:'smooth',block:'center'})}
...
['calcMargin','calcLev','calcSymbol','calcSide','calcEntry','calcTp','calcSl'].forEach(id=>$(id)?.addEventListener('input',updateCalc));
$('calcSide')?.addEventListener('change',updateCalc);$('tradeCalc')?.addEventListener('toggle',saveCalc);$('calcSymbol')?.addEventListener('input',e=>{const v=e.currentTarget.value.toUpperCase().replace(/[^A-Z0-9]/g,'');if(v!==e.currentTarget.value)e.currentTarget.value=v});loadCalc();
