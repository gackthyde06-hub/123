...
function renderTestCrossDetail(x){
  ...
  <div class="testCrossSection"><h4>資金 / 衍生品</h4><div class="testCrossGrid"><div><span>OI變化</span><b>${src.oi&&hasNum(lc.oiChangePct)?testSignedPct(lc.oiChangePct,1):'無資料'}</b></div><div><span>主動買賣比</span><b>${src.taker&&hasNum(lc.takerRatio)?Number(lc.takerRatio).toFixed(2):'無資料'}</b></div><div><span>大戶持倉 / 帳戶比</span><b>${esc(topText)}</b></div><div><span>全市場多空比</span><b>${src.globalLs&&globalLs?globalLs:'無資料'}</b></div><div><span>Funding / Basis</span><b>${src.funding&&fund?fund:'無資料'} / ${src.basis&&hasNum(lc.basisPct)?testSignedPct(lc.basisPct,3):'無資料'}</b></div><div><span>ADL Risk</span><b>${src.adl&&lc.adlRisk&&lc.adlRisk!=='unknown'?esc(String(lc.adlRisk).toUpperCase()):'無資料'}</b></div></div></div>
...
function renderTestSignals(d){
  ...
  if($('testHealth'))$('testHealth').textContent=`系統 ${healthText} · 最近掃描 ${scanSec==null?'—':scanSec+'秒前'} · 監控 ${Number(h.tracked||0)} · 資料完整度＋可信度共同影響通知級別`;
