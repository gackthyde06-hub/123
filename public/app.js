...
function positionRow(p,extra,open){const long=p.side==='LONG',pc=livePnlClass(p.pnlPct);return`<div class="pos ${extra&&!open?'hidden extraPos':extra?'extraPos':''}" data-calc-symbol="${esc(p.symbol)}" data-calc-side="${esc(p.side)}" data-calc-entry="${esc(p.entryPrice)}"><div class="symline"><button type="button" class="sym symTvLink" data-tv-symbol="${esc(p.symbol)}" aria-label="開啟 ${esc(p.symbol)} TradingView">${esc(p.symbol)}</button><button type="button" class="posCalcBtn" data-position-calc>試算</button><span class="dirTag ${long?'long':'short'}">${esc(p.direction)}</span></div><div class="posPnl ${pc}"><span class="openAt">${esc(positionOpenText(p.openTime))}</span><span class="pnlPct">${livePnlPct(p.pnlPct)}</span></div><div class="entryWrap"><span class="entryLabel">進場位</span><div class="price ${long?'red':'green'}">${price(p.entryPrice)}</div></div>${pullbackLine(p)}</div>`}
...
function renderIdeaAnalysisBody(d, symbol){
  ...
  return `<div class="ideaAnalysisStatus"><div><span>今日偏向</span><b class="${ideaBiasClass(d.bias)}">${esc(d.bias||'中性')}</b></div><div><span>強弱</span><b class="${ideaStrengthClass(d.strength)}">${esc(d.strength||'中性')}</b></div><div><span>更新</span><b>${localTime(d.generatedAt)||'剛剛'}</b></div></div><div class="ideaAnalysisProfile"><b>${esc(d.profile?.sector||'其他 / 新興資產')}</b><span>${esc(d.profile?.purpose||'')}</span></div>${d.summary?`<div class="ideaAnalysisSummary">${esc(d.summary)}</div>`:''}<div class="ideaAnalysisCols"><div><h4>利多</h4><ul>${bullish}</ul></div><div><h4>利空</h4><ul>${bearish}</ul></div></div><div class="ideaNews"><h4>今日消息</h4>${news}</div><div class="ideaTodayAction"><span>今天怎麼對待</span><b>${esc(d.action||'等結構確認，不追價。')}</b></div><div class="ideaAnalysisFoot">${d.mode==='AI_WEB'?'即時網搜＋Binance量化':'Binance量化'} · 同標的 2 小時快取 · 不追高/不追殺</div>`;
}
