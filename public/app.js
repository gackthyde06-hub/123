...
function renderTestFocus(){
  ...
  const cards=rows.map((x,i)=>{
    ...
    const dynamic=hasNum(x.monitorScore)?Number(x.monitorScore).toFixed(0):q,protect=hasNum(x.structureProtection)?price(x.structureProtection):(hasNum(x.stop)?price(x.stop):'—'),breakout=hasNum(x.breakoutLevel)?price(x.breakoutLevel):'—',topRatio=hasNum(x.monitorEvidence?.topPositionRatio)?Number(x.monitorEvidence.topPositionRatio).toFixed(2):'—';return `<details class="testMonitorItem ${isFocus?'focused':''}" data-test-judge="${esc(x.key)}" ${open?'open':''}><summary><div class="testMonitorSummaryMain"><span class="testMonitorSymbol">${i+1}. ${esc(x.symbol)}</span>${testTrendTag(x)}<span class="testMonitorDir ${long?'long':'short'}">${long?'做多':'做空'}</span><span class="testMonitorState">${esc(x.statusLabel||'未破確認')}</span></div><div class="testMonitorSummaryScore"><b>${winText}</b><span>勝率 · 動態 ${dynamic}</span></div></summary><div class="testMonitorBody"><div class="testFocusBody"><div class="testFocusCell"><span>同類1R勝率</span><b>${winText}</b></div><div class="testFocusCell"><span>動態強度</span><b>${dynamic}</b></div><div class="testFocusCell"><span>平均90分報酬</span><b>${avg}</b></div><div class="testFocusCell"><span>回測獲利因子</span><b>${pf}</b></div><div class="testFocusCell"><span>突破位</span><b>${breakout}</b></div><div class="testFocusCell"><span>目前保護結構</span><b>${protect}</b></div><div class="testFocusCell"><span>回踩區</span><b>${esc(testZoneText(x.setup))}</b></div><div class="testFocusCell"><span>確認價</span><b>${hasNum(x.confirmationPrice)?price(x.confirmationPrice):'等待確認'}</b></div><div class="testFocusCell"><span>大戶持倉多空比</span><b>${topRatio}</b></div></div><div class="testFocusMeta">${testReasonChips(x)||'等待回踩確認資料'} · 回測樣本 ${Number(bt.sample||0)} · 模型估算 ${hasNum(x.idea?.estimatedWinRate)?Number(x.idea.estimatedWinRate).toFixed(1)+'%':'—'}${hasNum(x.monitorEvidence?.weakFlags)?` · 轉弱條件 ${Number(x.monitorEvidence.weakFlags)}`:''}</div><div class="testFocusActions"><button type="button" class="closeJudge" data-test-collapse-one>縮小此筆</button><button type="button" class="openTv" data-tv-symbol="${esc(x.symbol)}">開啟 TV</button></div></div></details>`;
  }).join('');
