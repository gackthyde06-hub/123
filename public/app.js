...
const CALC_PREF='position-alert-order-calc-v600';
const TV_LAST_AUTO='position-alert-tv-last-auto-v600';
...
function saveCalc(){const o={margin:$('calcMargin')?.value||'',lev:$('calcLev')?.value||'',source:$('calcSource')?.value||'POSITION',position:$('calcPosition')?.value||'',tvSignal:$('calcTvSignal')?.value||'',tp:$('calcTp')?.value||'',sl:$('calcSl')?.value||'',open:$('tradeCalc')?.open||false};try{localStorage.setItem(CALC_PREF,JSON.stringify(o))}catch{}}
function loadCalc(){const d=loadObject(CALC_PREF,{});if($('calcMargin'))$('calcMargin').value=d.margin||'';if($('calcLev'))$('calcLev').value=d.lev||'';if($('calcTp'))$('calcTp').value=d.tp||'';if($('calcSl'))$('calcSl').value=d.sl||'';if($('calcSource'))$('calcSource').value=d.source==='TV'?'TV':'POSITION';if($('tradeCalc'))$('tradeCalc').open=!!d.open;if($('calcPosition'))$('calcPosition').dataset.saved=d.position||'';if($('calcTvSignal'))$('calcTvSignal').dataset.saved=d.tvSignal||'';setCalcSourceUI();updateCalc()}
function calcPositionKey(traderId,p){return `${traderId}|${p.symbol}|${p.side}`}
function monitoredPositions(status){const out=[];for(const t of status?.traders||[]){for(const p of t.positions||[]){if(!p?.symbol||!p?.side||!(Number(p.entryPrice)>0))continue;out.push({key:calcPositionKey(t.id,p),traderId:t.id,traderName:t.name,symbol:p.symbol,side:p.side,direction:p.direction||(p.side==='SHORT'?'做空':'做多'),entryPrice:Number(p.entryPrice)})}}return out}
function tvRows(status){return(status?.tradingView?.signals||[]).filter(s=>s?.id&&s?.symbol&&s?.side&&Number(s.entry)>0)}
function setCalcSourceUI(){const tv=$('calcSource')?.value==='TV';$('calcPositionWrap')?.classList.toggle('hidden',tv);$('calcTvWrap')?.classList.toggle('hidden',!tv);$('calcSourceBadge').textContent=tv?'TradingView 自動':'高手目前倉位'}
function clearCalcIdentity(){ $('calcSymbol').value='—';$('calcSide').value='LONG';$('calcSideLabel').value='—';$('calcEntry').value='—'; }
function renderCalcPositions(status){const sel=$('calcPosition');if(!sel)return;const rows=monitoredPositions(status),old=sel.value||sel.dataset.saved||'';sel.innerHTML=rows.length?'<option value="">請選擇一筆目前倉位</option>'+rows.map(x=>`<option value="${esc(x.key)}" data-symbol="${esc(x.symbol)}" data-side="${esc(x.side)}" data-entry="${esc(x.entryPrice)}">${esc(x.traderName)}｜${esc(x.symbol)}｜${esc(x.direction)}｜進場 ${price(x.entryPrice)}</option>`).join(''):'<option value="">目前沒有可選倉位</option>';if(old&&rows.some(x=>x.key===old))sel.value=old;else if(rows.length===1)sel.value=rows[0].key;sel.dataset.saved='';if($('calcSource')?.value!=='TV')applyCalcPosition(false)}
function renderTvSignals(status){const sel=$('calcTvSignal'),rows=tvRows(status);if(!sel)return;const old=sel.value||sel.dataset.saved||'';sel.innerHTML=rows.length?rows.map((s,i)=>`<option value="${esc(s.id)}" data-symbol="${esc(s.symbol)}" data-side="${esc(s.side)}" data-entry="${esc(s.entry)}" data-sl="${esc(s.sl??'')}" data-tp1="${esc(s.tp1??'')}" data-tp2="${esc(s.tp2??'')}" data-tp3="${esc(s.tp3??'')}">${i===0?'最新｜':''}${esc(s.symbol)}｜${esc(s.direction||s.side)}｜${price(s.entry)}${s.timeframe?`｜${esc(s.timeframe)}`:''}</option>`).join(''):'<option value="">等待 TradingView 訊號</option>';if(old&&rows.some(x=>x.id===old))sel.value=old;else if(rows.length)sel.value=rows[0].id;sel.dataset.saved='';const latest=rows[0],lastAuto=localStorage.getItem(TV_LAST_AUTO)||'';if(latest&&latest.id!==lastAuto){localStorage.setItem(TV_LAST_AUTO,latest.id);$('calcSource').value='TV';sel.value=latest.id;$('tradeCalc').open=true;setCalcSourceUI();applyTvSignal(true)}else if($('calcSource')?.value==='TV'){applyTvSignal(false)}renderTvStatus(status)}
function renderTvStatus(status){const box=$('tvStatus');if(!box)return;const tv=status?.tradingView||{},last=tv.lastSignalAt;if(last){box.innerHTML=`<span class="tvDot ok"></span><b>TV 已連線</b><span>最近訊號 ${ageText(last)}</span>`}else{box.innerHTML='<span class="tvDot"></span><b>TV 等待訊號</b><span>設定 Webhook 後會自動帶入</span>'}}
function applyCalcPosition(scroll=true){if($('calcSource')?.value==='TV')return;const sel=$('calcPosition'),opt=sel?.selectedOptions?.[0],symbol=opt?.dataset?.symbol||'',side=opt?.dataset?.side||'',entry=opt?.dataset?.entry||'';$('calcSymbol').value=symbol||'—';$('calcSide').value=side==='SHORT'?'SHORT':'LONG';$('calcSideLabel').value=symbol?(side==='SHORT'?'做空':'做多'):'—';$('calcEntry').value=entry||'—';if(symbol){$('calcTp').value='';$('calcSl').value=''}updateCalc();if(symbol){$('calcMsg').innerHTML=`<span class="calcSelected">已選 ${esc(opt.textContent||'')}</span> · 填金額、槓桿、TP / SL 即可試算`;if(scroll)$('tradeCalc').scrollIntoView({behavior:'smooth',block:'center'})}saveCalc()}
function applyTvSignal(scroll=true){if($('calcSource')?.value!=='TV')return;const opt=$('calcTvSignal')?.selectedOptions?.[0],symbol=opt?.dataset?.symbol||'',side=opt?.dataset?.side||'',entry=opt?.dataset?.entry||'',sl=opt?.dataset?.sl||'',tp1=opt?.dataset?.tp1||'',tp2=opt?.dataset?.tp2||'',tp3=opt?.dataset?.tp3||'';$('calcSymbol').value=symbol||'—';$('calcSide').value=side==='SHORT'?'SHORT':'LONG';$('calcSideLabel').value=symbol?(side==='SHORT'?'做空':'做多'):'—';$('calcEntry').value=entry||'—';$('calcSl').value=sl||'';$('calcTp').value=tp1||'';updateCalc();if(symbol){const extras=[tp2?`TP2 ${price(tp2)}`:'',tp3?`TP3 ${price(tp3)}`:''].filter(Boolean).join(' · ');$('calcMsg').innerHTML=`<span class="calcSelected">TV 已自動帶入 ${esc(symbol)} ${side==='SHORT'?'做空':'做多'}</span>${extras?` · ${esc(extras)}`:''}${(!sl||!tp1)?' · 此訊號未提供完整 TP/SL':''}`;if(scroll)$('tradeCalc').scrollIntoView({behavior:'smooth',block:'center'})}saveCalc()}
function updateCalc(){
 const margin=calcNum('calcMargin'),lev=calcNum('calcLev'),entry=calcNum('calcEntry'),tp=calcNum('calcTp'),sl=calcNum('calcSl'),side=$('calcSide')?.value||'LONG',msg=$('calcMsg');
 const validBase=margin>0&&lev>=1&&lev<=125,notional=validBase?margin*lev:null,qty=Number.isFinite(notional)&&entry>0?notional/entry:null;
 $('calcNotional').textContent=Number.isFinite(notional)?`${notional.toLocaleString('en-US',{maximumFractionDigits:1})} U`:'—';$('calcQty').value=fmtCalcQty(qty);
 $('calcProfit').textContent='—';$('calcLoss').textContent='—';$('calcProfitPct').textContent='—';$('calcLossPct').textContent='—';$('calcRR').textContent='—';msg.classList.remove('bad');
 if(!(margin>0)||!(lev>=1&&lev<=125)){msg.textContent='先輸入保證金與槓桿（1–125X）。';saveCalc();return}
 if(!(entry>0)||!(tp>0)||!(sl>0)){msg.textContent=$('calcSource')?.value==='TV'?'TV 已自動帶入訊號；若 Pine 有送 TP / SL 會一起填入。':'選倉位後輸入 TP / SL 即可試算。';saveCalc();return}
 ...
 const tpMove=side==='LONG'?tp-entry:entry-tp,slMove=side==='LONG'?entry-sl:sl-entry,profit=tpMove*qty,loss=slMove*qty,profitPct=profit/margin*100,lossPct=loss/margin*100,rr=loss>0?profit/loss:null;
 $('calcProfit').textContent=fmtU(profit);$('calcLoss').textContent=`-${Math.abs(loss).toLocaleString('en-US',{maximumFractionDigits:Math.abs(loss)>=100?1:2})} U`;$('calcProfitPct').textContent=`+${profitPct.toFixed(1)}%`;$('calcLossPct').textContent=`-${Math.abs(lossPct).toFixed(1)}%`;$('calcRR').textContent=Number.isFinite(rr)?`1 : ${rr.toFixed(2)}`:'—';
 msg.textContent=`${$('calcSymbol').value.trim().toUpperCase()||'標的'} · ${side==='LONG'?'做多':'做空'} · 總倉位 ${notional.toLocaleString('en-US',{maximumFractionDigits:1})} U`;saveCalc();
}
function fillCalcFromPosition(el){if(!el)return;const sym=String(el.dataset.calcSymbol||'').toUpperCase(),side=el.dataset.calcSide||'',entry=String(el.dataset.calcEntry||''),sel=$('calcPosition');$('calcSource').value='POSITION';setCalcSourceUI();if(sel){const opt=[...sel.options].find(o=>o.dataset.symbol===sym&&o.dataset.side===side&&String(o.dataset.entry||'')===entry)||[...sel.options].find(o=>o.dataset.symbol===sym&&o.dataset.side===side);if(opt)sel.value=opt.value}$('tradeCalc').open=true;applyCalcPosition(true)}
...
function renderTvSetup(){if(!cfg)return;const u=$('tvWebhookUrl'),m=$('tvWebhookMessage');if(u)u.value=cfg.tvWebhookUrl||'';if(m)m.value=cfg.tvBasicMessage||'';}
...
['calcMargin','calcLev','calcTp','calcSl'].forEach(id=>$(id)?.addEventListener('input',updateCalc));
$('calcSource')?.addEventListener('change',()=>{setCalcSourceUI();$('calcSource').value==='TV'?applyTvSignal(false):applyCalcPosition(false);saveCalc()});
$('calcPosition')?.addEventListener('change',()=>applyCalcPosition(false));
$('calcTvSignal')?.addEventListener('change',()=>applyTvSignal(false));
$('tradeCalc')?.addEventListener('toggle',saveCalc);loadCalc();
