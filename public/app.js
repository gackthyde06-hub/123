...
function tradingViewTicker(symbol){const clean=String(symbol||'').toUpperCase().replace(/[^A-Z0-9]/g,'');return clean?`BINANCE:${clean}.P`:''}
...
function tradingViewAppLink(symbol){const ticker=tradingViewTicker(symbol);return ticker?`tradingview://chart/?symbol=${encodeURIComponent(ticker)}`:'tradingview://'}
function showTvAppHint(symbol){let box=document.getElementById('tvAppHint');if(!box){box=document.createElement('div');box.id='tvAppHint';box.className='tvAppHint';document.body.appendChild(box)}box.innerHTML=`<span>TV 沒有自動開啟？</span><button type="button">再開一次</button>`;box.querySelector('button').onclick=()=>openTradingViewApp(symbol);box.classList.add('show');clearTimeout(showTvAppHint.timer);showTvAppHint.timer=setTimeout(()=>box.classList.remove('show'),6000)}
function openTradingViewApp(symbol){const ticker=tradingViewTicker(symbol);if(!ticker)return false;let leftPage=false;const watch=()=>{if(document.visibilityState==='hidden'){leftPage=true;clearTimeout(timer);document.removeEventListener('visibilitychange',watch)}};document.addEventListener('visibilitychange',watch);const timer=setTimeout(()=>{document.removeEventListener('visibilitychange',watch);if(!leftPage&&document.visibilityState==='visible')showTvAppHint(symbol)},1300);location.href=tradingViewAppLink(symbol);return false}
