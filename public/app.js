...
function renderTestFocus(){
  const panel=$('testFocusPanel');if(!panel)return;
  if(!testSignalsState){if(testFocusSymbol){panel.classList.add('show');panel.innerHTML='<div class="testMonitorTitle">策略判讀</div><div class="testMonitorSub">載入訊號中…</div>'}else{panel.classList.remove('show');panel.innerHTML=''}return}
  ...
  panel.innerHTML=`<div class="testMonitorHeader"><div class="testMonitorHeadLeft"><div class="testMonitorTitle">策略判讀</div><div class="testMonitorSub">前 3 筆＝完成度＋勝率優先 · WebSocket 現價約 1 秒 · 深度約 100ms · 多策略判讀約 30 秒</div></div><div class="testMonitorControls"><button type="button" data-test-expand-all>全部展開</button><button type="button" data-test-collapse-all>全部縮小</button></div></div><div class="testMonitorList">${topCards}</div>${moreHtml}${renderReachedPool(reached)}`;
...
function renderPerformance(d){
  ...
  $('perfByTier').innerHTML=perfBreakHtml(sum.byTier);$('perfByRegime').innerHTML=perfBreakHtml(sum.byRegime);if($('perfByStrategy'))$('perfByStrategy').innerHTML=perfBreakHtml(sum.byStrategy);$('perfBySymbol').innerHTML=perfBreakHtml(sum.bySymbol);
