...
async function chartResolveContext(symbol){let x=chartBestSignal(symbol);if(!x){try{const r=await fetch('/api/test-signals',{cache:'no-store'}),d=await r.json();if(r.ok&&d?.ok){testSignalsState=d;x=chartBestSignal(symbol)}}catch{}}if(x){const z=x.preferredEntryZone||x.entryZone||x.strategyProfile?.entryZone||null,zl=chartFinite(z?.low),zh=chartFinite(z?.high),best=zl&&zh?(zl+zh)/2:chartFinite(x.reentryEntryPrice||x.confirmationPrice||x.currentPrice),stop=chartFinite(x.reentryStop||x.structureProtection||x.stop||x.strategyProfile?.invalidation),dir=x.direction==='SHORT'?'SHORT':'LONG',sign=dir==='SHORT'?-1:1,risk=best&&stop?Math.abs(best-stop):null,tp1=chartFinite(x.reentryTarget1R||x.target1R)||(best&&risk?best+sign*risk:null),tp2=chartFinite(x.target15R)||(best&&risk?best+sign*risk*1.5:null);return {symbol,direction:dir,status:x.monitorLabel||x.statusLabel||'觀察中',strategy:testStrategyName(x),winRate:testEffectiveWinRate(x),bestEntry:best,zoneLow:zl,zoneHigh:zh,stop,tp1,tp2,current:chartFinite(x.currentPrice),notified:Boolean(x.notificationSentAt)}}
...
function pageSwipeBlockedTarget(target){
  ...
  return Boolean(target.closest('input,textarea,select,button,a,summary,label,[contenteditable="true"],.pageTabs,.modalBackdrop.show,.sheet,.tvAppHint.show,.chartModal.show,.chartShell'));
}
