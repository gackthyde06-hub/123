...
function positionRow(p,extra,open){const long=p.side==='LONG',pc=livePnlClass(p.pnlPct);return`<div class="pos ${extra&&!open?'hidden extraPos':extra?'extraPos':''}" data-calc-symbol="${esc(p.symbol)}" data-calc-side="${esc(p.side)}" data-calc-entry="${esc(p.entryPrice)}"><div class="symline"><button type="button" class="sym symTvLink" data-tv-symbol="${esc(p.symbol)}" aria-label="開啟 ${esc(p.symbol)} TradingView">${esc(p.symbol)}</button><span class="dirTag ${long?'long':'short'}">${esc(p.direction)}</span></div><div class="posPnl ${pc}"><span class="openAt">${esc(positionOpenText(p.openTime))}</span><span class="pnlPct">${livePnlPct(p.pnlPct)}</span></div><div class="entryWrap"><span class="entryLabel">進場位</span><div class="price ${long?'red':'green'}">${price(p.entryPrice)}</div></div>${pullbackLine(p)}</div>`}
...
async function loadIdeaAnalysis(details){
  ...
  const cached=ideaAnalysisCache.get(key);if(cached&&Date.now()-cached.at<2*60*60*1000){body.innerHTML=renderIdeaAnalysisBody(cached.data,symbol);details.dataset.loaded='1';return}
...
function renderTestCrossDetail(x){
  const bt=x.setup?.backtest||{},lc=x.lastCheck||{},ev=x.monitorEvidence||{},long=x.direction==='LONG',win=testEffectiveWinRate(x),confirmed=hasNum(x.confirmedWinRate)?Number(x.confirmedWinRate):null;
  ...
  const top=hasNum(lc.topPositionRatio)?Number(lc.topPositionRatio).toFixed(2):hasNum(ev.topPositionRatio)?Number(ev.topPositionRatio).toFixed(2):'—',globalLs=hasNum(lc.globalLongShortRatio)?Number(lc.globalLongShortRatio).toFixed(2):'—';
  const depth=hasNum(lc.depthImbalance)?`${Number(lc.depthImbalance)>0?'+':''}${(Number(lc.depthImbalance)*100).toFixed(1)}%`:'—',fund=hasNum(lc.fundingPct)?`${Number(lc.fundingPct).toFixed(4)}%`:'—';
  const state=testFreshnessState(x)==='STALE'?'資料過期':testFreshnessState(x)==='DELAYED'?'資料延遲':(x.monitorLabel||x.statusLabel||'等待');
  const risks=[];if(ev.adverse15)risks.push('15分逆向');if(ev.adverse30)risks.push('30分逆向');if(ev.adverse1h)risks.push('1小時逆向');if(lc.fundingCrowded)risks.push('Funding擁擠');if(String(lc.adlRisk||'').toLowerCase()==='high')risks.push('ADL高風險');if(hasNum(lc.spreadBps)&&Number(lc.spreadBps)>8)risks.push('價差偏大');if(hasNum(lc.chaseAtr)&&Number(lc.chaseAtr)>.35)risks.push('距離過遠/追價');
  const supports=[];if(lc.reclaim)supports.push('回踩收回');if(lc.sweep)supports.push('掃流動性後收回');if(lc.wicker)supports.push('拒絕影線');if(lc.momentum)supports.push('短線動能同向');if(lc.macdImprove)supports.push('MACD改善');if(Number(lc.marketAlign)>0)supports.push('BTC/ETH同向');if((long&&Number(lc.takerRatio)>=1)||( !long&&hasNum(lc.takerRatio)&&Number(lc.takerRatio)<=1))supports.push('主動買賣未逆向');
  return `<div class="testCrossIntro"><div><span>目前狀態</span><b>${esc(state)}</b></div><div><span>當日排名</span><b>${x.rank?`#${x.rank}`:'—'} · 熱度 ${hasNum(x.rankHeat)?Number(x.rankHeat).toFixed(0):'—'}</b></div><div><span>校準勝率</span><b>${win>=0?win.toFixed(1)+'%':'—'}${confirmed!=null&&win>=0?` · ${win-confirmed>=0?'+':''}${(win-confirmed).toFixed(1)}`:''}</b></div><div><span>進場策略</span><b>${esc(x.entryStrategy||'等回踩確認，不追價')}</b></div></div>
  <div class="testCrossSection"><h4>多週期結構 / 動能</h4><div class="testCrossGrid"><div><span>5分 RSI</span><b>${hasNum(lc.rsi5)?Number(lc.rsi5).toFixed(1):'—'}</b></div><div><span>15分 ADX</span><b>${hasNum(x.setup?.adx15)?Number(x.setup.adx15).toFixed(1):'—'}</b></div><div><span>30分方向</span><b>${testTrendWord(lc.t30Trend)}</b></div><div><span>1小時方向</span><b>${testTrendWord(lc.h1Trend)}</b></div><div><span>成交量比</span><b>${hasNum(lc.volumeRatio)?Number(lc.volumeRatio).toFixed(2):'—'}</b></div><div><span>MACD改善</span><b>${testBoolWord(lc.macdImprove)}</b></div></div></div>
  <div class="testCrossSection"><h4>資金 / 衍生品</h4><div class="testCrossGrid"><div><span>OI變化</span><b>${testSignedPct(lc.oiChangePct,1)}</b></div><div><span>主動買賣比</span><b>${hasNum(lc.takerRatio)?Number(lc.takerRatio).toFixed(2):'—'}</b></div><div><span>大戶持倉比</span><b>${top}</b></div><div><span>全市場多空比</span><b>${globalLs}</b></div><div><span>Funding</span><b>${fund}</b></div><div><span>ADL風險</span><b>${esc(String(lc.adlRisk||'—').toUpperCase())}</b></div></div></div>
  <div class="testCrossSection"><h4>委託簿 / 進場品質</h4><div class="testCrossGrid"><div><span>20檔深度失衡</span><b>${depth}</b></div><div><span>價差</span><b>${hasNum(lc.spreadBps)?Number(lc.spreadBps).toFixed(2)+' bps':'—'}</b></div><div><span>追價距離</span><b>${hasNum(lc.chaseAtr)?Number(lc.chaseAtr).toFixed(2)+' ATR':'—'}</b></div><div><span>BTC/ETH對齊</span><b>${Number(lc.marketAlign)>0?'同向':Number(lc.marketAlign)<0?'逆向':'中性'}</b></div><div class="wide"><span>較佳進場區</span><b>${esc(zone)}</b></div><div><span>保護 / 失效</span><b>${protect}</b></div></div></div>
  <div class="testCrossSection"><h4>歷史 / 實測</h4><div class="testCrossGrid"><div><span>歷史1R</span><b>${hasNum(bt.hitRate)?Number(bt.hitRate).toFixed(1)+'%':'—'}</b></div><div><span>回測樣本</span><b>${Number(bt.sample||0)}</b></div><div><span>獲利因子</span><b>${hasNum(bt.profitFactor)?Number(bt.profitFactor).toFixed(2):'—'}</b></div><div><span>平均90分報酬</span><b>${testSignedPct(bt.avgReturnPct,2)}</b></div><div><span>MFE</span><b>${testSignedPct(x.mfePct,2)}</b></div><div><span>MAE</span><b>${testSignedPct(x.maePct,2)}</b></div></div></div>
...
async function loadTestDeepAnalysis(details){
  if(!details||details.dataset.webLoaded==='1'||details.dataset.webLoading==='1')return;const symbol=details.dataset.testDetailSymbol||'',direction=details.dataset.testDetailDir||'',body=details.querySelector('[data-test-web-analysis-body]');if(!body)return;
  const key=`${symbol}:${direction}`,cached=ideaAnalysisCache.get(key);if(cached&&Date.now()-cached.at<2*60*60*1000){body.innerHTML=renderIdeaAnalysisBody(cached.data,symbol);details.dataset.webLoaded='1';return}
  details.dataset.webLoading='1';body.innerHTML='<div class="ideaAnalysisLoading">正在用網路消息＋Binance量化交叉比對…</div>';
  try{const d=await fetchIdeaAnalysisShared(symbol,direction);body.innerHTML=renderIdeaAnalysisBody(d,symbol);details.dataset.webLoaded='1'}catch(e){body.innerHTML='<div class="ideaAnalysisLoading">網路交叉分析暫時不可用；上方即時量化仍持續更新。</div>'}finally{details.dataset.webLoading='0'}
}
function bindTestDeepDetails(){const root=$('testGrid');if(!root)return;bindPersistentDetails(root);root.querySelectorAll('details[data-test-deep-detail]').forEach(d=>{if(d.dataset.deepBound!=='1'){d.dataset.deepBound='1';d.addEventListener('toggle',()=>{if(d.open)void loadTestDeepAnalysis(d)})}if(d.open)void loadTestDeepAnalysis(d)})}
...
function renderMonitorJudgeCard(x,i,focusKey,reLiveText){
  ...
  return `<details class="testMonitorItem ${isFocus?'focused':''} ${stale?'dataStale':''}" data-test-judge="${esc(x.key)}" ${open?'open':''}>
    ...
    <div class="testMonitorBody">
      ...
      <div class="testFocusActions"><button type="button" class="closeJudge" data-test-collapse-one>縮小此筆</button><button type="button" class="removeJudge" data-test-dismiss="${esc(x.key)}">移除判讀</button><button type="button" class="openTv" data-tv-symbol="${esc(x.symbol)}">開啟 TV</button></div>
    </div>
  </details>`;
}
...
function renderReachedPool(rows){
  ...
  const items=rows.map(x=>{const stale=testIsStale(x),win=testEffectiveWinRate(x),re=x.reentryStage==='WAIT_PULLBACK'?'等二次回踩':x.reentryStage==='WIN'?'二進達標':x.reentryStage==='FAILED'?'二進失效':'達標',zone=hasNum(x.reentryZoneLow)&&hasNum(x.reentryZoneHigh)?`${price(x.reentryZoneLow)}～${price(x.reentryZoneHigh)}`:'尚未形成';return `<div class="reachedRow ${stale?'dataStale':''}"><div><b>${esc(x.symbol)}</b><span>排名 ${Number(x.rank||0)||'—'} · 達標 ${localTime(x.targetReachedAt)} · 更新 ${testLastEvalTime(x)} · ${re}</span></div><div><strong>${stale?'—':win>=0?win.toFixed(1)+'%':'—'}</strong><small>${stale?'資料過期':'二踩 '+zone}</small></div></div>`}).join('');
...
function renderTestFocus(){
  ...
  panel.innerHTML=`<div class="testMonitorHeader"><div class="testMonitorHeadLeft"><div class="testMonitorTitle">回踩判讀</div><div class="testMonitorSub">前 3 筆優先 · 現價約 10 秒更新 · 判讀約 60 秒更新</div></div><div class="testMonitorControls"><button type="button" data-test-expand-all>全部展開</button><button type="button" data-test-collapse-all>全部縮小</button></div></div><div class="testMonitorList">${topCards}</div>${moreHtml}${renderReachedPool(reached)}`;
