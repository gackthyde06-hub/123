...
function localTime(iso){if(!iso)return'';try{return new Date(iso).toLocaleTimeString('zh-TW',{hour:'2-digit',minute:'2-digit',hour12:false,hourCycle:'h23'})}catch{return''}}
...
function testMonitorCandidates(){
  ...
  return [...picked.values()].sort((a,b)=>Number(b.priorityScore||0)-Number(a.priorityScore||0)||testEffectiveWinRate(b)-testEffectiveWinRate(a)||(a.rank||99)-(b.rank||99)||Number(b.qualityScore||0)-Number(a.qualityScore||0));
}
...
function renderTestFocus(){
  ...
  const cards=rows.map((x,i)=>{
    ...
    const rankNow=Number(x.rank||0),rankConfirm=Number(x.rankAtConfirm||0),rankText=rankNow?`排名 ${rankNow}`:'排名 —',priority=hasNum(x.priorityScore)?Number(x.priorityScore).toFixed(0):'—',heat=hasNum(x.rankHeat)?Number(x.rankHeat).toFixed(0):'—';
    ...
    const reMeta=(x.reentryReasons||[]).length?` · 二進：${(x.reentryReasons||[]).map(esc).join('、')}`:'';
    return `<details class="testMonitorItem ${isFocus?'focused':''}" data-test-judge="${esc(x.key)}" ${open?'open':''}><summary><div class="testMonitorSummaryMain"><span class="testMonitorSymbol">${i+1}. ${esc(x.symbol)}</span><span class="testMonitorRankTag">${rankText}</span>${testTrendTag(x)}<span class="testMonitorDir ${long?'long':'short'}">${long?'做多':'做空'}</span><span class="testMonitorState">${esc(x.statusLabel||'未破確認')}</span><span class="testMonitorEventTime">${eventTime}</span>${invalidWindow?`<span class="testMonitorRecover">${esc(invalidWindow)}</span>`:''}</div><div class="testMonitorSummaryScore"><b>${winText}</b><span>校準勝率 · 優先 ${priority}</span></div></summary><div class="testMonitorBody"><div class="testFocusBody"><div class="testFocusCell"><span>校準勝率</span><b>${winText}</b></div><div class="testFocusCell"><span>監控優先分</span><b>${priority}</b></div><div class="testFocusCell"><span>當日排名 / 熱度</span><b>${rankNow?`${rankNow}`:'—'} / ${heat}</b></div><div class="testFocusCell"><span>確認時排名</span><b>${rankConfirm?`${rankConfirm}`:'—'}</b></div><div class="testFocusCell"><span>保守區間</span><b>${ciLow}～${ciHigh}</b></div><div class="testFocusCell"><span>歷史1R勝率</span><b>${hist}</b></div><div class="testFocusCell"><span>可信度 / 等效樣本</span><b>${esc(confidence)} / ${Number(x.winRateMeta?.effectiveSample||0)}</b></div><div class="testFocusCell"><span>動態強度</span><b>${dynamic}</b></div><div class="testFocusCell"><span>平均90分報酬</span><b>${avg}</b></div><div class="testFocusCell"><span>回測獲利因子</span><b>${pf}</b></div><div class="testFocusCell"><span>突破位</span><b>${breakout}</b></div><div class="testFocusCell"><span>目前保護結構</span><b>${protect}</b></div><div class="testFocusCell"><span>回踩區</span><b>${esc(testZoneText(x.setup))}</b></div><div class="testFocusCell"><span>確認價</span><b>${hasNum(x.confirmationPrice)?price(x.confirmationPrice):'等待確認'}</b></div><div class="testFocusCell"><span>二次回踩區</span><b>${reZone}</b></div><div class="testFocusCell"><span>二次條件分</span><b>${reScore}</b></div><div class="testFocusCell"><span>二進實測勝率 / 樣本</span><b>${reLiveText}</b></div><div class="testFocusCell"><span>二次進場 / 停損</span><b>${reEntry} / ${reStop}</b></div><div class="testFocusCell"><span>二次 1R</span><b>${reTarget}</b></div><div class="testFocusCell"><span>大戶持倉多空比</span><b>${topRatio}</b></div><div class="testFocusCell"><span>20檔委託簿失衡</span><b>${depth}</b></div><div class="testFocusCell"><span>1小時趨勢</span><b>${h1}</b></div><div class="testFocusCell"><span>狀態發生時間</span><b>${eventTime}</b></div></div><div class="testFocusMeta">${invalidReason?`${esc(invalidReason)} · `:''}${testReasonChips(x)||'等待回踩確認資料'} · 歷史回測 ${Number(bt.sample||0)}筆 / ${Number(bt.bars||0)}根5分K · APP實測加權 ${Number(x.winRateMeta?.liveSample||0)}筆 · 波動環境 ${esc(x.winRateMeta?.regime||x.setup?.volatilityRegime||'—')}${hasNum(x.monitorEvidence?.weakFlags)?` · 轉弱條件 ${Number(x.monitorEvidence.weakFlags)}`:''}${reMeta}</div><div class="testFocusActions"><button type="button" class="closeJudge" data-test-collapse-one>縮小此筆</button><button type="button" class="openTv" data-tv-symbol="${esc(x.symbol)}">開啟 TV</button></div></div></details>`;
  }).join('');
  panel.classList.add('show');
  panel.innerHTML=`<div class="testMonitorHeader"><div class="testMonitorHeadLeft"><div class="testMonitorTitle">回踩判讀</div><div class="testMonitorSub">通知訊號 ${rows.length} 筆 · 當日建議熱度＋校準勝率排序</div></div><div class="testMonitorControls"><button type="button" data-test-expand-all>全部展開</button><button type="button" data-test-collapse-all>全部縮小</button></div></div><div class="testMonitorList">${cards}</div>`;
...
function renderTestSignals(d){
  ...
  const active=rows.filter(x=>['WAIT_PULLBACK','TOUCHING','CONFIRMED','INVALID'].includes(x.status)).length,touching=rows.filter(x=>x.status==='TOUCHING').length,confirmed=rows.filter(x=>x.status==='CONFIRMED').length,liveHit=hasNum(live.hitRate)?`${Number(live.hitRate).toFixed(1)}%`:'—';
  ...
  $('testGrid').innerHTML=rows.map(x=>{const bt=x.setup?.backtest||{},q=Math.max(0,Math.min(100,Number(x.qualityScore||x.setup?.setupScore||0))),long=x.direction==='LONG',hist=hasNum(bt.hitRate)?`${Number(bt.hitRate).toFixed(1)}%`:'—',avg=testSignedPct(bt.avgReturnPct,2),pf=hasNum(bt.profitFactor)?Number(bt.profitFactor).toFixed(2):'—',model=hasNum(x.idea?.estimatedWinRate)?`${Number(x.idea.estimatedWinRate).toFixed(1)}%`:'—',rsi=hasNum(x.lastCheck?.rsi5)?Number(x.lastCheck.rsi5).toFixed(1):'—',oi=testSignedPct(x.lastCheck?.oiChangePct,1);return `<div class="testCard ${testStatusClass(x.status)}"><div class="testHead"><div class="testRank">${x.rank?`排名 ${x.rank}`:'排名 —'}</div><div class="testSymbolRow"><span class="testSymbol">${esc(x.symbol)}</span>${testTrendTag(x)}<span class="testDir ${long?'long':'short'}">${long?'做多':'做空'}</span></div><div class="testState"><b>${esc(x.statusLabel||'等待')}</b><small>${x.updatedAt?ageText(x.updatedAt):'—'}</small></div></div><div class="testQuality"><div class="testQualityBar"><div class="testQualityFill" style="width:${q}%"></div></div><div class="testQualityText">品質 <b>${q||'—'}</b></div></div><div class="testMetrics"><div class="testMetric"><span>同類1R勝率</span><b>${hist}</b></div><div class="testMetric"><span>回測樣本</span><b>${Number(bt.sample||0)}</b></div><div class="testMetric"><span>平均90分報酬</span><b>${avg}</b></div><div class="testMetric"><span>回測獲利因子</span><b>${pf}</b></div><div class="testMetric"><span>模型估算</span><b>${model}</b></div><div class="testMetric"><span>5分 RSI / OI</span><b>${rsi} / ${oi}</b></div><div class="testMetric"><span>動態強度</span><b>${hasNum(x.monitorScore)?Number(x.monitorScore).toFixed(0):q}</b></div><div class="testMetric"><span>大戶持倉多空比</span><b>${hasNum(x.monitorEvidence?.topPositionRatio)?Number(x.monitorEvidence.topPositionRatio).toFixed(2):hasNum(x.lastCheck?.topPositionRatio)?Number(x.lastCheck.topPositionRatio).toFixed(2):'—'}</b></div></div><div class="testLevels"><div class="testLevel"><span>回踩區</span><b>${esc(testZoneText(x.setup))}</b></div><div class="testLevel"><span>失效參考</span><b>${x.setup?price(x.setup.invalidation):'—'}</b></div><div class="testLevel"><span>確認價</span><b>${hasNum(x.confirmationPrice)?price(x.confirmationPrice):'等待確認'}</b></div><div class="testLevel"><span>1R / 1.5R</span><b>${hasNum(x.target1R)?`${price(x.target1R)} / ${price(x.target15R)}`:'確認後計算'}</b></div><div class="testLevel"><span>突破位</span><b>${hasNum(x.breakoutLevel)?price(x.breakoutLevel):'確認後計算'}</b></div><div class="testLevel"><span>目前保護結構</span><b>${hasNum(x.structureProtection)?price(x.structureProtection):(hasNum(x.stop)?price(x.stop):'確認後計算')}</b></div></div><div class="testReasons">${testReasonChips(x)||'<span class="testReason">等待5分回踩確認</span>'}</div><div class="testActions"><button type="button" class="monitorBtn" data-test-monitor="${esc(x.symbol)}" data-test-dir="${esc(x.direction)}">到監控判讀</button><button type="button" class="openTvBtn" data-tv-symbol="${esc(x.symbol)}">開啟TV</button></div></div>`}).join('')||'<div class="testEmpty">目前建議排名沒有可追蹤標的。</div>';
  $('testMethod').textContent=`判讀：當日排名＋5/15/60分結構＋ADX/RSI/MACD＋OI/主動買賣/大戶持倉＋20檔委託簿＋BTC/ETH。達1R後不追價，至少等2根5分K，再等38.2–61.8二次回踩與EMA/POC/VWAP共振；連續2根確認才提示二次進場。失效可在${Number(d.reactivateMinutes||30)}分內重新轉強；連續 ${Number(d.badBars||3)} 根5分K變爛且高週期同步弱，或30分未收復才移出。`;
