...
try{patchResearchLayer();researchLayerReady=true}catch(err){console.error('[ui:v256] Research R1 patch skipped:',String(err?.message||err))}
...
for(const name of files){
  ...
  if(!fs.existsSync(source))throw new Error(`[ui:v256] missing ${name}`);
...
const cssTags=[
  '<link rel="stylesheet" href="/system-growth.css?v=sg256">',
  '<link rel="stylesheet" href="/premium-theme.css?v=sg256">',
  '<link rel="stylesheet" href="/structure-engine-v2.css?v=sg256">',
].join('\n');
const jsTags=[
  '<script defer src="/system-growth.js?v=sg256"></script>',
  '<script defer src="/premium-theme.js?v=sg256"></script>',
  '<script defer src="/structure-engine-v2-ui.js?v=sg256"></script>',
].join('\n');
html=html.replace('</head>',`${cssTags}\n</head>`);
html=html.replace('</body>',`${jsTags}\n</body>`);
fs.writeFileSync(htmlPath,html,'utf8');
console.log(`[ui:v256] clean rebase ready · Research R1=${researchLayerReady?'ready':'skipped'} · Structure=S2.1.0 · testSignals=${stability.changed?'nonblocking':'already-nonblocking'} · rescueLayers=OFF`);
